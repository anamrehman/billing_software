...
class bill_app:
    def __init__(self,root):
        ...
        search_btn=Button(F1,text="Search",command=self.find_bill,width=15,bd=7,font="arial 12 bold").grid(row=0,column=8,padx=10,pady=10)
        ...
        F5.place(x=1070,y=190,width=440,height=380)
        ...
        F7=LabelFrame(self.root,bd=10,relief=GROOVE,font=("times new roman",15,"bold"),fg="gold",bg=bg_color)
        F7.place(x=1050,y=580,width=460,height=70)
        print_btn=Button(F7,text="Print",command=self.print,width=15,bd=7,font="arial 12 bold").grid(row=0,column=5,padx=140,pady=5)
    ...
    def bill_area(self):
        ...
        #=======COSMETICS============
            if self.bath_soap.get()!=0:
                self.txtarea.insert(END,f"\n BATH SOAP\t\t\t{self.bath_soap.get()}\t\t{self.b_s_p}")
            ...
            if self.hair_spray.get()!=0:
                self.txtarea.insert(END,f"\n HAIR SPRAY\t\t\t{self.hair_spray.get()}\t\t{self.h_s_p}")
            ...
            if self.shampoo.get()!=0:
                self.txtarea.insert(END,f"\n SHAMPOO\t\t\t{self.shampoo.get()}\t\t{self.s_p}")
            ...
            if self.coak.get()!=0:
                self.txtarea.insert(END,f"\n COAK\t\t\t{self.coak.get()}\t\t{self.cd_co_p}")

            if self.frooti.get()!=0:
                self.txtarea.insert(END,f"\n FROOTI\t\t\t{self.frooti.get()}\t\t{self.cd_fr_p}")

            if self.thumbsup.get()!=0:
                self.txtarea.insert(END,f"\n THUMBUPS\t\t\t{self.thumbsup.get()}\t\t{self.cd_tu_p}")

            if self.pepsi.get()!=0:
                self.txtarea.insert(END,f"\n PEPSI\t\t\t{self.pepsi.get()}\t\t{self.cd_pe_p}")

            if self.sprite.get()!=0:
                self.txtarea.insert(END,f"\n SPRITE\t\t\t{self.sprite.get()}\t\t{self.cd_sp_p}")

            if self.slice.get()!=0:
                self.txtarea.insert(END,f"\n SLICE\t\t\t{self.slice.get()}\t\t{self.cd_sl_p}")
